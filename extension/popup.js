...
function renderTable(tabs) {
  ...
  const total = Math.max(1, state.data?.counts?.totalTabs || tabs.length);
  const totalMem = state.data?.memory?.usedBytes || 0;
  const estPerTabBytes = totalMem > 0 ? Math.round(totalMem / total) : null;

  const rows = tabs.map(tab => {
    const estMemText = estPerTabBytes ? `${formatBytes(estPerTabBytes)}` : 'n/a';
    const cpuText = tab.active ? 'active' : 'idle';
    const netText = /^https?:/.test(tab.url) ? 'web' : 'local';
    return `<tr>
      <td><div class="task-col"><img src="${escapeHtml(tab.favIconUrl || 'icons/icon16.png')}" onerror="this.src='icons/icon16.png'" alt=""><span>${escapeHtml(tab.title)}</span></div></td>
      <td>${escapeHtml(tab.domain || '')}</td>
      <td class="metric-col">${formatDuration(tab.inactiveSeconds)}</td>
      <td class="metric-col">${estMemText}</td>
      <td class="metric-col">${escapeHtml(cpuText)}</td>
      <td class="metric-col">${escapeHtml(netText)}</td>
      <td class="metric-col">${tab.discarded ? 'Yes' : 'No'}</td>
      <td class="metric-col">${tab.eligible ? 'Yes' : 'No'}</td>
      <td>
  ...
  tabList.innerHTML = `<div class="table-wrap"><table class="tab-table"><thead><tr>
    <th style="width:34%"><button class="th-sort" data-sort="title_asc">Task</button></th>
    <th style="width:12%">Domain</th>
    <th style="width:10%"><button class="th-sort" data-sort="inactive_desc">Inactive</button></th>
    <th style="width:10%">Memory*</th>
    <th style="width:8%">CPU*</th>
    <th style="width:8%">Net*</th>
    <th style="width:6%">Susp</th>
    <th style="width:6%"><button class="th-sort" data-sort="eligible_desc">Elig</button></th>
    <th style="width:16%">Actions</th>
  </tr></thead><tbody>${rows}</tbody></table></div>
  <div class="event-time" style="margin-top:6px;">*Memory is per-tab estimate from system memory; CPU/Net are lightweight state hints.</div>`;
